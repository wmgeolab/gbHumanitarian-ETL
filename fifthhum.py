...
def create_folders(Country_iso):
    ...
    for adm_level in range(6):
        ...
        for extension in ext:
            files_with_extension = [file for file in os.listdir(extrct_path) if file.lower().endswith(f".{extension}") and adm_string in file and name_string in file]
...
@flow(name='UNOCHA: ETLFive',flow_run_name=generate_flow_run_name,log_prints=True)
